 ...
 */
public abstract class StreamParameterTransformation<T> implements ParameterTransformation<T> {
	final public T transform(String arg) throws ModuleException {
		try (InputStream is = IOUtils.toInputStream(arg, "UTF-8")) {
			return transform(is);
		} catch (IOException e) {
	 ...
	 */
	abstract public T transform(InputStream arg) throws ModuleException, IOException;
}
