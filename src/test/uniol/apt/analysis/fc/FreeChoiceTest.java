...
public class FreeChoiceTest {
	...
	private boolean testNet(PetriNet pn) throws PreconditionFailedException {
		return new FreeChoice().check(pn);
	}

	private Boolean testNetModule(PetriNet pn) {
		try {
			FCModule module = new FCModule();
