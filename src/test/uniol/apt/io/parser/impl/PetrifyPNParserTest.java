...
public class PetrifyPNParserTest {
	...
	public void testIsolatedPlace() throws Exception {
		...
		assertEquals(pn.getPlaces().size(), 1);
		assertEquals(pn.getEdges().size(), 0);
	}
	...
	public void testFlowWeightExplicitOwn() throws Exception {
		...
		assertEquals(pn.getPlaces().size(), 1);
		assertEquals(pn.getEdges().size(), 2);
		assertEquals(pn.getPostsetEdges("a"), Collections.emptySet());
		assertEquals(pn.getPostsetEdges("b"), Collections.emptySet());
		for (Flow flow : pn.getPresetEdges("a"))
			assertEquals(flow.getWeight(), 1);
		for (Flow flow : pn.getPresetEdges("b"))
			assertEquals(flow.getWeight(), 2);
	...
	public void testFlowWeightImplicitOwn() throws Exception {
		...
		assertEquals(pn.getPlaces().size(), 1);
		assertEquals(pn.getEdges().size(), 2);
		assertEquals(pn.getPresetEdges("a"), Collections.emptySet());
		assertEquals(pn.getPostsetEdges("b"), Collections.emptySet());
		for (Flow flow : pn.getPostsetEdges("a"))
			assertEquals(flow.getWeight(), 2);
		for (Flow flow : pn.getPresetEdges("b"))
