...
public class PrimeCycleProperty {
	public ParikhVector check(TransitionSystem ts) {
		Set<Pair<List<String>, ParikhVector>> parikhs = new ComputeSmallestCycles().computePVsOfSmallestCycles(ts);

		for (Pair<List<String>, ParikhVector> cycle : parikhs) {
			ParikhVector pv = cycle.getSecond();
